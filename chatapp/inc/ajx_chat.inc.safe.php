...
class SimpleAjaxyChat {
    ...
    */
    function getMessages($bOnlyMessages = false) {
	 $roomme = $GLOBALS['myroom'];
        $aMessages = $GLOBALS['MySQL']->getAll("SELECT `s_ajax_chat_messages`.*, `s_members`.`name`, UNIX_TIMESTAMP()-`s_ajax_chat_messages`.`when` AS 'diff' FROM `s_ajax_chat_messages` INNER JOIN `s_members` ON `s_members`.`id` = `s_ajax_chat_messages`.`member_id`  where room = '".$roomme."' ORDER BY `id` DESC LIMIT 15");
        $sMessages = '';
